...
public class Main {
	...
	public static  void printUserFunction(Admin admin)
	{
		ServiceProvider sp;
		Service service;
	...
	public static void printAdminFunction(Admin admin)
	{
		...
		else if(opt==2)
		{
			...
			System.out.println("2-Specfic Discount");
			opt=input.nextInt();
			if(opt==1)
			{
				OverallDiscount discount=new OverallDiscount();
				System.out.println("Enter the discount percentage");
				float dis=input.nextFloat();
				discount.addDiscount(dis);
			}
			else if(opt==2)
			{
				SpecificDiscount discount=new SpecificDiscount();
				System.out.println("Enter the discount percentage");
				float dis=input.nextFloat();
				System.out.println("Enter the service name");
				String service=input.next();
				discount.addDiscount(dis,service);
			}
		}
	}
